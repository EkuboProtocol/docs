...
How the sqrt ratio is _encoded_ differs between the Starknet and EVM deployments.
...
A full-precision 64.128 sqrt ratio needs 192 bits, which doesn't leave room for anything else in a 256-bit storage word. The AMM doesn't actually need 192 bits of precision at every magnitude — it needs _relative_ precision. By encoding the sqrt ratio in 96 bits, the entire pool state packs into a **single storage slot**:
