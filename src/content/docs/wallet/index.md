...
An agent can inspect public wallet state, simulate an action, submit a request, and wait for its result. It cannot approve a request, export a private key, install a signing policy, or change security-sensitive settings. Those actions stay in the native application and require the owner where appropriate.
