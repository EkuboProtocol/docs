---
description: Control which wallet transactions are allowed, denied, or sent to native review
...
:::caution[Policies are not scoped to a requester]
Rules match transaction fields, not the MCP client, harness, automation, dapp, WalletConnect session, or displayed plan source. An allow rule written for an agent also allows the same matching transaction when an installed automation or connected dapp requests it. Personal-message and typed-data signatures remain native-review-only.
:::
...
:::danger
An unrestricted allow rule permits any transaction that an agent, installed automation, or connected dapp can request from that account. Use the narrowest rule that expresses the recurring action you intend.
:::
